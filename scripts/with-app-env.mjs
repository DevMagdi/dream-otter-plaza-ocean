...
/** File values under the process environment: an explicit process-env override wins. */
...
/**
 * Resolve a local npm bin without relying on PATH.
 * Windows `spawn('vite')` looks for vite.exe and misses vite.cmd (ENOENT).
 * Running the JS entry through node works with spaces in the project path.
 */
export function resolveLocalCommand(command, root = projectRoot()) {
  const jsBins = {
    vite: join(root, "node_modules", "vite", "bin", "vite.js"),
  };
  const js = jsBins[command];
  if (js && existsSync(js)) {
    return { command: process.execPath, argsPrefix: [js], shell: false };
  }
  const binDir = join(root, "node_modules", ".bin");
  if (process.platform === "win32") {
    const cmd = join(binDir, `${command}.cmd`);
    if (existsSync(cmd)) return { command: cmd, argsPrefix: [], shell: true };
  } else {
    const unix = join(binDir, command);
    if (existsSync(unix)) return { command: unix, argsPrefix: [], shell: false };
  }
  return { command, argsPrefix: [], shell: process.platform === "win32" };
}
...
export function withLocalBinPath(env, root = projectRoot()) {
  ...
  const next = { ...env };
  const keys = new Set(
    Object.keys(next).filter((key) => key.toLowerCase() === "path"),
  );
  if (keys.size === 0) keys.add("PATH");
  for (const key of keys) {
    next[key] = `${binDir}${delimiter}${next[key] || ""}`;
  }
  return next;
}
...
function main(argv) {
  ...
  const resolved = resolveLocalCommand(command, root);
