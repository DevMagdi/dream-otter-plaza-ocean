...
class LazyPGliteDriver implements Driver {
  ...
  async init(): Promise<void> {
    try {
      const { mssqlWanted } = await import("@/lib/ppe/local-config");
      if (mssqlWanted()) {
  ...
  async acquireConnection(): Promise<DatabaseConnection> {
    if (this.client === undefined) {
      try {
        const { mssqlWanted } = await import("@/lib/ppe/local-config");
        if (mssqlWanted()) return new EmptyConnection();
      } catch {
        /* continue */
      }
      this.client = await this.getClient();
  ...
  async releaseConnection(connection: DatabaseConnection): Promise<void> {
    if (connection instanceof EmptyConnection) return;
  ...
  async beginTransaction(
    ...
    if (conn instanceof EmptyConnection) return;
...
class EmptyConnection implements DatabaseConnection {
  async executeQuery<O>(_compiledQuery: CompiledQuery): Promise<QueryResult<O>> {
    return { rows: [] };
  }
