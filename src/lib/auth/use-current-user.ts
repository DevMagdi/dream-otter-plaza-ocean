...
import { currentActor } from "@/lib/ppe/db-status";
...
export function useCurrentUserState(): CurrentUserState {
  ...
  const [sqlUser, setSqlUser] = useState<AppUser | null | undefined>(undefined);
  // eslint-disable-next-line react-hooks/rules-of-hooks
  useEffect(() => {
    void currentActor()
      .then((row) => {
      ...
      .catch(() => setSqlUser(null));
  }, []);
  return {
    user: sqlUser ?? (data?.user
      ? {
          id: data.user.id,
